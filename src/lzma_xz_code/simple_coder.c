...
// xz-5.0.1/src/liblzma/simple/simple_coder.c:20: //
static lzma_ret
...
// xz-5.0.1/src/liblzma/simple/simple_coder.c:54: //
...
// xz-5.0.1/src/liblzma/simple/simple_coder.c:65: //
static lzma_ret
	    ...
	    size_t out_size, lzma_action action)
{
	...
	if (coder->pos < coder->filtered) {
		lzma_bufcpy(coder->buffer, &coder->pos, coder->filtered, out,
			    out_pos, out_size);
	...
	if (out_avail > buf_avail) {
		...
		*out_pos += buf_avail;
		...
		const size_t filtered =
		    call_filter(coder, out + out_start, size);
	...
	if (coder->size > 0) {
		...
		coder->filtered =
		    call_filter(coder, coder->buffer, coder->size);
...
// xz-5.0.1/src/liblzma/simple/simple_coder.c:188: //
...
// xz-5.0.1/src/liblzma/simple/simple_coder.c:198: //
static lzma_ret
...
// xz-5.0.1/src/liblzma/simple/simple_coder.c:206: //
extern lzma_ret
lzma_simple_coder_init(lzma_next_coder * next, lzma_allocator * allocator,
		       const lzma_filter_info * filters,
		       size_t(*filter) (lzma_simple * simple, uint32_t now_pos,
					bool is_encoder, uint8_t * buffer,
					size_t size), size_t simple_size,
		       size_t unfiltered_max, uint32_t alignment,
		       bool is_encoder)
{
	...
	if (next->coder == NULL) {
		...
		if (simple_size > 0) {
			next->coder->simple =
			    lzma_alloc(simple_size, allocator);
